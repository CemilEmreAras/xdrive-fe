...
function CarList() {
  ...
  useEffect(() => {
    ...
    if (filters.category && filters.category.length > 0) {
      filtered = filtered.filter(car => {
        ...
        return filters.category.includes(carGroupStr)
      })
    }
    ...
    if (filters.transmission && filters.transmission.length > 0) {
      filtered = filtered.filter(car => {
        ...
        const isManual = carTransmission.includes('manuel') || carTransmission.includes('manual') || carTransmission === 'm'
        const isAutomatic = carTransmission.includes('otomatik') || carTransmission.includes('automatic') || carTransmission.includes('auto') || carTransmission === 'o'

        if (filters.transmission.includes('Manual') && isManual) return true
        if (filters.transmission.includes('Automatic') && isAutomatic) return true
        return false
      })
    }
    ...
    filtered.sort((a, b) => {
      ...
      if (filters.sortBy === 'price') {
        const calculatePrice = (car) => {
          const totalPrice = parseFloat(car.totalPrice || car.Total_Rental || car.total_Rental || 0)
          const pricePerDay = parseFloat(car.pricePerDay || car.Daily_Rental || car.daily_Rental || 0)
          // Display logic uses: totalPrice > 0 ? totalPrice : pricePerDay * days
          return totalPrice > 0 ? totalPrice : pricePerDay * days
        }

        aValue = calculatePrice(a)
        bValue = calculatePrice(b)
      }
  ...
  const handleFilterChange = (key, value) => {
    if (key === 'category' || key === 'transmission') {
      setFilters(prev => {
  ...
  const getRentalConditionsUrl = () => {
    ...
    const configs = [
      ...
      },
      {
        ...
        file: 'kenya-vehicle-rental-terms-conditions.pdf'
      },
      {
      ...
      {
        keywords: ['turkey', 'türkiye', 'istanbul', 'ankara', 'izmir', 'antalya', 'bodrum', 'dalaman', 'montenegro', 'karadağ', 'podgorica', 'tivat', 'budva'],
        file: 'turkey-montenegro-vehicle-rental-terms-conditions.pdf'
      },
      {
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-5 md:px-5 box-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-5">
            ...
            <aside className="w-full md:w-auto">
              {/* Filter Panel */}
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit md:sticky md:top-5 mt-5 md:static">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  <h3 className="m-0 text-xl font-bold text-gray-900">{t('carList.filter')}</h3>
                  <button
                    className="bg-none border-none text-blue-600 text-sm font-medium cursor-pointer p-0 hover:text-blue-700 transition-colors"
                ...
                <div className="mb-6">
                  <div
                    className="flex justify-between items-center cursor-pointer select-none mb-3 group"
                    onClick={() => toggleSection('carType')}
                  >
                    <h4 className="m-0 text-base font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{t('carList.carType')}</h4>
                    <svg
                      className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${expandedSections.carType ? 'rotate-180' : ''}`}
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${expandedSections.carType ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="flex flex-col gap-3 pl-1">
                      {availableCategories.length > 0 ? (
                          ...
                          <label key={category} className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative flex items-center">
                              <input
                                ...
                                className="peer appearance-none w-5 h-5 border-2 border-gray-300 rounded bg-white checked:bg-blue-600 checked:border-blue-600 transition-all cursor-pointer"
                              />
                              <svg
                            ...
                            <span className="text-sm text-gray-600 font-medium group-hover:text-gray-900 transition-colors select-none">
                              {category}
                            </span>
                          </label>
                        ))
                        ...
                        // Fallback
                        <>
                          {['MINI', 'ECONOMY', 'COMPACT', 'INTERMEDIATE', 'STANDARD'].map(cat => (
                ...
                <div>
                  ...
                  >
                    <h4 className="m-0 text-base font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{t('carList.transmission')}</h4>
                    <svg
                      className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${expandedSections.transmission ? 'rotate-180' : ''}`}
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${expandedSections.transmission ? 'max-h-[200px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="flex flex-col gap-3 pl-1">
                      ...
                      <label className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="text-sm text-gray-600 font-medium group-hover:text-gray-900 transition-colors select-none">
                          {t('common.automatic').toUpperCase()}
                        </span>
                      </label>
                    </div>
                  </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[300px_250px_1fr] gap-6 items-start">
                          ...
                          <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-2 p-3 bg-[#f9fafb] rounded-md mt-1">
                              ...
                              <div className="flex flex-col gap-0.5 mt-1 border-t border-[#e5e7eb] pt-2 w-full">
                                <div className="flex items-center gap-2 text-[13px] text-[#333] font-medium">
                                  <svg className="flex-shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M19.77 7.23L19.78 7.22L16.06 3.5L15 4.56L17.11 6.67C16.17 7.03 15.5 7.93 15.5 9C15.5 10.38 16.62 11.5 18 11.5C18.36 11.5 18.69 11.42 18.98 11.29L19 21H21V9H19.77ZM7 2H13V4H7V2ZM7 6H13V8H7V6ZM7 10H13V12H7V10ZM5 2C3.9 2 3 2.9 3 4V20C3 21.1 3.9 22 5 22H19C20.1 22 21 21.1 21 20V4C21 2.9 20.1 2 19 2H5ZM5 20V4H19V20H5Z" fill="#0066FF" />
                                  </svg>
                                  <span className="text-[#666]">{t('carList.fuelPolicy')}:</span>
                                </div>
                                <div className="pl-[26px] text-sm font-bold text-[#1a1a1a]">
                                  {t('carList.sameToSame')}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2.5 mt-1">
                              ...
                              <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 mt-2">
                                {/* Mileage */}
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  <svg className="flex-shrink-0 w-4 h-4 text-[#84cc16]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  ...
                                  <span>{t('carList.collisionDamageWaiver')}</span>
                                </div>
                                {/* Theft Waiver */}
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  <svg className="flex-shrink-0 w-4 h-4 text-[#84cc16]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  ...
                                  <span>{t('carList.theftWaiver')}</span>
                                </div>
                                {/* TPL */}
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  <svg className="flex-shrink-0 w-4 h-4 text-[#84cc16]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  ...
                                  <span>{t('carList.thirdPartyLiability')}</span>
                                </div>
                                {/* GST */}
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  <svg className="flex-shrink-0 w-4 h-4 text-[#84cc16]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#4b5563]">
                                  ...
                                  <span>{t('carList.airportFee')}</span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-3 pt-4 mt-1 border-t border-[#e5e7eb]">
                              ...
                              <div className="flex items-end gap-2 text-[#1a1a1a]">
                                <span className="text-2xl sm:text-3xl md:text-[32px] font-bold leading-none">{currencySymbol}{finalPrice.toFixed(2)}</span>
                                <span className="text-sm text-[#666] mb-1 font-medium">
                                  ({currencySymbol}{(finalPrice / days).toFixed(2)} / {t('carList.day')})
                                </span>
                              </div>
                              <Link
                                ...
                                className="block w-full py-3 text-base font-semibold text-center text-white transition-colors bg-[#0066FF] rounded-md hover:bg-blue-700"
                              >
                                {t('carList.bookNow')}
                              </Link>
                            </div>
